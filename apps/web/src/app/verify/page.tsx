...
export default function VerifyPage() {
  ...
  // Download markdown certificate generated by the Fileverse publish pipeline.
  const handleDownloadProof = async () => {
    if (!proofArtifacts) {
      setManualError("No proof artifacts found in this session.");
      return;
    }

    const certificatePath = proofArtifacts.jobId
      ? `/api/proofs/${proofArtifacts.jobId}/certificate`
      : proofArtifacts.vkHash
        ? `/api/proofs/legacy/certificate?txHash=${encodeURIComponent(proofArtifacts.vkHash)}`
        : null;

    if (!certificatePath) {
      setManualError("Certificate markdown is unavailable for this proof. Please regenerate proof from the App page.");
      return;
    }
    ...
    setManualError(null);
    try {
      const res = await fetch(certificatePath);
      if (!res.ok) {
      ...
      if (!data?.markdown || typeof data.markdown !== "string") {
        throw new Error("Certificate markdown was not returned");
      }
      ...
      a.href = url;
      a.download = `proofos-certificate-${new Date().toISOString().split("T")[0]}.md`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (err) {
      setManualError(err instanceof Error ? err.message : "Failed to download certificate");
    } finally {
      ...
      <div className="max-w-3xl mx-auto px-4 py-16 pb-32">
        ...
        {!isConnected && (
          <div className="mb-6 rounded-xl border border-amber-800/50 bg-amber-950/30 p-4 flex items-center gap-3">
            <IconWallet className="w-5 h-5 text-amber-400 shrink-0" />
            ...
            <div className="rounded-2xl border border-emerald-800/50 bg-emerald-950/20 p-6">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  {isDownloadingCertificate ? "Preparing Certificate..." : "Download Certificate (.md)"}
                </button>
                <button
